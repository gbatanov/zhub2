...
Устройство управления - Raspberry Pi 4 64x Ubuntu 20 server aarch64

## Примечания к проекту
- На маках папка по умолчанию для поиска программ и библиотек /opt/local, в отличие от Linux, где /usr/local
- 38 ножка задействована для определения наличия 220В. Реле подключается к БП 220В/5В.
- Средний контакт на 38 ножку гребенки, НЗ контакт на общий провод, НО на +3,3В через резистор 30кОм.
...
- Для токена телеграм-бота создавать файл .token в который необходимо скопировать токен. При инсталляции этот файл скопируется в папку /usr/local/etc/zhub2 
...
|-----------|--------------|--------------------------------|
| AT+CPAS   | +CPAS:0      | Информация о состоянии модуля: |
|           | OK           | 0 - готов к работе             |
|           |              | 2 - неизвестно |
|           |              | 3 - входящий звонок |
...
| ATD+79250109365; | OK        |   Позвонить на номер +79250109365|
...
|                    |             |                     При включенном АОН:|
|                  | +CLIP: "+79250109365",145,"",,"",0 | Номер телефона,(другие параметры мне не интересны)|
...
## TO-DO list:
- перевести обработку событий в классы конечных устройств

## Инструкция по созданию клона SD-карты на маке
### Создание клона

1. Вставьте SD-карту в ваш Mac с помощью USB или встроенного кард-ридера. Теперь откройте окно терминала и введите команду diskutil list . Попробуйте определить идентификатор устройства вашей SD-карты. (Например, /dev/disk5 )
...
- 2022.09.03 Версия 2.5.200 Отработка команд из смс. Тег v2.5
- 2022.08.13 Версия 2.4.174 на сегодня принята за рабочую и залита в мастер с тегом v2.4. С версии 2.5.175 начинаю проработку асинхронных запросов-ответов, разборку с самописными событиями и их обработчиками.
