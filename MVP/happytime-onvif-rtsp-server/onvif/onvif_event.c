	...
	for (i=0; i<MAX_NUM_EUA; i++)
	{
		...
		else if (p_eua->pullmsg.used_flag)
		{
		        ...
		        msgnums >= p_eua->pullmsg.req.MessageLimit)
		    {
		        ...
		        soap_build_send_rly(p_cln, NULL, build_tev_PullMessages_rly_xml, (char *)&p_eua->pullmsg.req, 
...
BOOL onvif_init_Message(onvif_Message * p_req, onvif_PropertyOperation op, const char * sname1, const char * svalue1, const char * sname2, const char * svalue2, const char *dname1, const char * dvalue1, const char * dname2, const char * dvalue2)
{
...
void onvif_send_simulate_events(const char * topic)
{
    ...
    {
        NotificationMessageList * p_message = onvif_init_NotificationMessage3(topic, PropertyOperation_Initialized, "VideoSource", "V_SRC_000", "RuleName", "TestMotionRegion", "State", "true", NULL, NULL);
		if (p_message)
		{
			onvif_put_NotificationMessage(p_message);
		}
    ...
    {
#ifdef PROFILE_C_SUPPORT
        AreaInfoList * p_area_info = g_onvif_cfg.area_info;
        while (p_area_info)
        {
            ...
            {
                p_message = onvif_init_NotificationMessage3(topic, PropertyOperation_Initialized, "AreaToken", p_area_info->AreaInfo.token, NULL, NULL, NULL, NULL, NULL, NULL);
            }
