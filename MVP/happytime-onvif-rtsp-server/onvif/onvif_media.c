...
ONVIF_RET onvif_trt_GetVideoEncoderConfigurationOptions(trt_GetVideoEncoderConfigurationOptions_REQ * p_req, trt_GetVideoEncoderConfigurationOptions_RES * p_res)
{
	...
	// Return global default options here
    memcpy(p_res, &g_onvif_cfg.VideoEncoderConfigurationOptions, sizeof(onvif_VideoEncoderConfigurationOptions));
...
ONVIF_RET onvif_trt_GetStreamUri(const char * lip, uint32 lport, trt_GetStreamUri_REQ * p_req, trt_GetStreamUri_RES * p_res)
{
	...
	if (p_profile->stream_uri[0] == '\0')
	{
	    ...
	    {
	        offset += sprintf(p_res->MediaUri.Uri, "http://%s:%d/test.mp4", lip, lport);
	    }
	    else
	    {
	        offset += sprintf(p_res->MediaUri.Uri, "rtsp://%s:%d/test.mp4", lip, g_onvif_cls.rtsp_port);
        }
	    ...
	    if (p_profile->a_dec_cfg)
        {
	        ...
	        offset += snprintf(p_res->MediaUri.Uri+offset, len-offset, "&amp;bce=%s", encoding);
	    }
#endif
	}
	else
