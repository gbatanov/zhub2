...
int rtp_tcp_tx(RSUA * p_rua, void * p_data, int len)
{
    ...
    if (p_rua->rtsp_send)
    {
        HTTPCLN * p_user = (HTTPCLN *) p_rua->rtsp_send;
...
int rtp_udp_tx(RSUA * p_rua, int av_t, char * p_rtp_data, int len)
{
    ...
    while (p_rua->rtp_tx && offset < len)
    {
    	int tlen = sendto(fd, p_rtp_data+offset, len-offset, 0, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));
        if (tlen > 0)
        {
            offset += tlen;
        }
...
int rtp_build(RSUA * p_rua, int av_t, uint8 * p_data, int len, uint32 ts, int mbit)
{
	...
	if (p_rua->rtp_tcp)
	{
		...
		*(buff+offset) = p_rua->channels[av_t].interleaved; // channel
        ...
        if (p_rua->replay)
        {
            *(uint16*)(buff+offset) = htons(12 + len + 16); // rtp payload length
        }
        else
        {
	...
	*(buff+offset) = (p_rua->channels[av_t].rtp_info.rtp_pt) | ((mbit & 0x01) << 7);
	offset++;
	*(uint16*)(buff+offset) = htons((uint16)(p_rua->channels[av_t].rtp_info.rtp_cnt));
	offset += 2;
	*(uint32*)(buff+offset) = htonl(p_rua->channels[av_t].rtp_info.rtp_ts);
	offset += 4;
	*(uint32*)(buff+offset) = htonl(p_rua->channels[av_t].rtp_info.rtp_ssrc);
	offset += 4;
    ...
    if (p_rua->replay)
    {
        ...
        *(uint32*)(buff+offset) = htonl(p_rua->channels[av_t].rep_hdr.ntp_sec);
        offset += 4;
        *(uint32*)(buff+offset) = htonl(p_rua->channels[av_t].rep_hdr.ntp_frac);
        offset += 4;
        ...
        *(buff+offset) = p_rua->channels[av_t].rep_hdr.seq;
    	offset++;
    	*(uint16*)(buff+offset) = htons(p_rua->channels[av_t].rep_hdr.padding);
    	offset += 2;
	...
	{
		slen = rtp_udp_tx(p_rua, av_t, (char *)p_rtp_ptr, offset+len);
		if (slen != offset+len)
		{
			return -1;
		}
...
int rtp_h264_single_fu_build(RSUA * p_rua, uint8 nalu_t, int fu_s, int fu_e, uint8 * p_data, int len)
{
	...
	if (p_rua->rtp_tcp)
	{
		...
		*(buff+offset) = p_rua->channels[AV_VIDEO_CH].interleaved;  // channel
        ...
        if (p_rua->replay)
        {
            *(uint16*)(buff+offset) = htons(12 + 2 + len + 16); // rtp payload length
        }
        else
        {
	...
	*(buff+offset) = (p_rua->channels[AV_VIDEO_CH].rtp_info.rtp_pt) | ((mbit & 0x01) << 7);
	offset++;
	*(uint16*)(buff+offset) = htons((uint16)(p_rua->channels[AV_VIDEO_CH].rtp_info.rtp_cnt));
	offset += 2;
	*(uint32*)(buff+offset) = htonl(p_rua->channels[AV_VIDEO_CH].rtp_info.rtp_ts);
	offset += 4;
	*(uint32*)(buff+offset) = htonl(p_rua->channels[AV_VIDEO_CH].rtp_info.rtp_ssrc);
	offset += 4;
    ...
    if (p_rua->replay)
    {
        ...
        *(uint32*)(buff+offset) = htonl(p_rua->channels[AV_VIDEO_CH].rep_hdr.ntp_sec);
        offset += 4;
        *(uint32*)(buff+offset) = htonl(p_rua->channels[AV_VIDEO_CH].rep_hdr.ntp_frac);
        offset += 4;
        ...
        *(buff+offset) = p_rua->channels[AV_VIDEO_CH].rep_hdr.seq;
    	offset++;
    	*(uint16*)(buff+offset) = htons(p_rua->channels[AV_VIDEO_CH].rep_hdr.padding);
    	offset += 2;
