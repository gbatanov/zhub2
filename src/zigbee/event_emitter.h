...
// event_command_ отслеживает поступление ответов на отправленную команду (T - Command)
...
public:
    ...
    void emit(int event_id, Command argument)
    {
    ...
    // Если оно произошло, возвращаем сохраненный в событии аргумент, иначе по тайм-ауту возвращаем nullopt
    std::optional<Command> wait(int event_id, std::chrono::duration<int, std::milli> timeout)
    {
        Listener listener = getListener(event_id);
...
private:
    ...
    Listener getListener(int event_id)
    {
        ...
        if (!listeners.count(event_id))
            listeners.insert(std::pair<int, Listener>(event_id, {std::make_shared<Event>(), std::make_shared<Command>()}));
